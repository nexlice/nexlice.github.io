...
#### 주의점
콜백 함수를 연결할 떄 함수 실행을 연결하는 것이 아닌, 함수 자체를 연결해야한다.  
...
### Why Async
`Node.js`는 asynchronous event-driven JavaScript runtime이며, thread없이 디자인 되었다.  
동기적 : 클라이언트가 서버에게 요청을 보냈을 때, 서버가 요청을 받는 상황에서  
클라이언트는 아무것도 하지 않고 기다린다.  
서버가 응답을 보내면 그때부터 클라이언트는 일을 한다.  

비동기적 : 클라이언트가 서버에게 요청을 한 이후에도 자신의 일을 한다.  

이를테면 YouTube를 볼 떄 영상이 로드 되는 동안 아무것도 못한다고 한다면 동기적이다.
로딩 되는 동안 다른 활도을 할 수 있다면 비동기적이다.

### Controlling Async
비동기로 여러 태스크를 할 떄, 각각의 태스크가 걸리는 시간이 다를 수 있기 때문에  
결과를 실행시간에만 알 수 있다.  
...
### aysnc wait
async/wait은 promise와 동일한 기능을 하지만, syntactic 추가이다.  
...
이 값을 작업이 끝나고 사용할 수 있다.    
.then(param)을 통해서 사용 가능하다.  

### promise의 then, catch
resolve가 되면 then으로 넘어가고,
